<?php
require('kernel/class/cache.php');
...
require('kernel/class/riotTournaments.php');
class riotapi {
	const API_URL = 'https://{region}.api.riotgames.com/lol/{endpoint}/v{version}/';
	const API_URL_MASTERY = 'https://{region}.api.riotgames.com/championmastery/location/{platform}/player/';
	const API_URL_FEATURED = 'https://{region}.api.riotgames.com/observer-mode/rest/featured';
	const API_URL_STATIC = 'https://global.api.riotgames.com/api/lol/static-data/{region}/v1.2/';
	const API_URL_SHARDS = 'http://status.leagueoflegends.com/shards';
	const API_URL_CURRENT_GAME = 'https://{region}.api.riotgames.com/observer-mode/rest/consumer/getSpectatorGameInfo/';
		...
		if (is_array($summonerId) && count($summonerId) > 1) {
			...
			if(count($summonerId) > $this->API_LIMIT_LEAGUES)
			{
				...
				foreach($summonerIds as $summonersChunked)
				{
					...
					$call = str_replace(array('{version}','{endpoint}'),array($leagueVersion, $endpoint),self::API_URL) . $call;
					$return .= $this->request($call,$dbPath,$dbFile,$dbTime,$region);
				}
				return $return;
			}
	...
	public function match($matchId, $region, $timeLine=false) {
		...
		$leagueVersion = '3';
		$endpoint = 'match';
		$call = str_replace(array('{version}','{endpoint}'),array($leagueVersion, $endpoint),self::API_URL) . 'matches/' . $matchId;
		if($timeLine)
		{
	...
	public function matchHistory($summonerId,$region) {
		...
		$endpoint = 'match';
		$call = str_replace(array('{version}','{endpoint}'),array('3', $endpoint),self::API_URL) . 'matchlists/by-account/' . $summonerId;
		return $this->request($call,$dbPath,$dbFile,$dbTime,$region);
	}
